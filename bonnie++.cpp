...
TestFileOps(int file_size, CGlobalItems &globals)
{
  if(file_size)
  {
    ...
    if(char_io_chunks)
    {
      ...
      globals.timer.add_latency(ByteWrite, dur.getMean());
    ...
    globals.timer.add_latency(FastWrite, dur.getMean());
    if(!globals.quiet) fprintf(stderr, "done\n");
    ...
    globals.timer.add_latency(ReWrite, dur.getMean());
    if(!globals.quiet) fprintf(stderr, "done\n");
    ...
    if(char_io_chunks)
    {
      ...
      globals.timer.add_latency(ByteRead, dur.getMean());
      if(!globals.quiet) fprintf(stderr, "done\n");
    }
    ...
    globals.timer.add_latency(FastRead, dur.getMean());
    if(!globals.quiet) fprintf(stderr, "done\n");
